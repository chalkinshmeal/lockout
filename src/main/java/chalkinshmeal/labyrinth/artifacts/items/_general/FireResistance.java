...
public class FireResistance extends LabyrinthItem {
    public FireResistance(JavaPlugin plugin, ConfigHandler config, String className, String position, Game game) { super(plugin, config, className, position, game); }

    public void onSpawn(Player player) {
        if (!isPlayerHoldingItemInInventory(player)) return;

        giveEffect(player, FIRE_RESISTANCE, 1000000, 1);
    }
