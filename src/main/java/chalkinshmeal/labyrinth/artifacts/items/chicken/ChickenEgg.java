...
public class ChickenEgg extends LabyrinthItem {
    public ChickenEgg(JavaPlugin plugin, ConfigHandler config, String className, String position, Game game) { super(plugin, config, className, position, game); }
    ...
    public void onPlayerInteractEvent(PlayerInteractEvent event) {
        ...
        projectile.setItem(new ItemStack(EGG));
    ...
    public void onProjectileHitEvent(ProjectileHitEvent event) {
        ...
        if (!isProjectileAnItem(projectile, EGG)) return;
        ...
        ((Player) shooter).getWorld().createExplosion(tntSpawnLoc, 1);
