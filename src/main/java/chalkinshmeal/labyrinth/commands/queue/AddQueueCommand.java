...
public class AddQueueCommand extends ArgCommand {
    private final JavaPlugin plugin;
    private final ConfigHandler config;
    private final LabyrinthHandler labyrinthHandler;

    // Constructor
    public AddQueueCommand(JavaPlugin plugin, LabyrinthHandler labyrinthHandler) {
        super("add-queue", true);
        this.setPermission("labyrinth.add-queue");
        setPlayerRequired(true);
        this.setHelpMsg(ChatColor.GOLD + this.getName() + " [labyrinth name]: " +
                        ChatColor.WHITE + "Adds a queue to an Labyrinth at the current position");
        this.plugin = plugin;
    ...
    protected void executeArgs(CommandSender _sender, List<ArgValue> argValues, Set<String> usedFlags) {
        ...
        this.labyrinthHandler.addQueue(player, labyrinthName);
    }
}
