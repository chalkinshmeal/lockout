...
public class ClearChestCommand extends ArgCommand {
    private final JavaPlugin plugin;
    private final ConfigHandler config;
    private final LabyrinthHandler labyrinthHandler;

    // Constructor
    public ClearChestCommand(JavaPlugin plugin, LabyrinthHandler labyrinthHandler) {
        super("clear-chests", true);
        this.setPermission("labyrinth.clear-chests");
        setPlayerRequired(true);
        this.setHelpMsg(ChatColor.GOLD + this.getName() + " [labyrinth name]: " +
                        ChatColor.WHITE + "Clears all chestss in an Labyrinth");
        this.plugin = plugin;
    ...
    protected void executeArgs(CommandSender _sender, List<ArgValue> argValues, Set<String> usedFlags) {
        ...
        this.labyrinthHandler.clearChests(player, labyrinthName);
    }
}
